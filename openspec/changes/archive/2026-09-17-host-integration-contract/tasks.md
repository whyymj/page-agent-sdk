...
> **收尾复核(2026-09-19,归档前)**:4.18 发布后历经 4.19-4.22 四版(输出纪律/浮层菜单/hostWatch/proposals 均动了提示词与工具面),归档前重跑 docs-qa 全套复核 —— 4 场景 10 断言中 9 绿,S3 两处断言语义修正后全绿:①`verified_before_answer` 放宽为「本轮读 或 同会话早前真读过且零宿主变更」(4.19 输出纪律后模型在 S2 整页读后会**正确地复用**而非冗余重读 —— 按 stale-read 设计语义〔仅写/换文后失效〕这是合法行为,原断言过严);②`honest_absence` 正则补「没有出现/没有出现任何/不涉及」形态(模型三次运行三种诚实缺失措辞全为正确行为)。**结论:4.18 机制面在 4.22 头上零回归,误伤率维持 0(任务 20 判据),归档。**
