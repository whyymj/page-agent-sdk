...
> 状态:**✅ 已实施(2026-09-07,待发布;全 9 任务收口;D1 = 主包+headless 双入口同带 / D2 = 自用迁移已做 —— waitIdle·diffBaseline 改消费导出层,真场景 S10 复验 + --baseline-diff 自比全持平)**。优先级 P3(工程/生态)。目标:把 SDK 自用的真 LLM 回归方法论(idle 判定/baseline 对比/场景套件)抽成**纯函数判定层**公开导出,让集成方(editor_fangzhou 类)在自己项目里为自己的宿主场景跑升级前回归 —— 生态粘性投资。
