...
> 状态:**已回退归档(2026-08-22)**。Phase 1/2 曾随 3.41.0 发布(`data.tools` 白名单 + `vfs.mainTools`),Phase 3(editor 接入)评估时集成方决策成本实测过高(需理解「旧四件 vs 高层套」「主栈 vfs 与子 agent 池关系」等 SDK 内部史才能正确决策,收益仅每轮几 K token schema)→ 维护者拍板「不需要这种会让用户疑惑的配置项」,**3.43.0 移除两个选项**,SDK 回到零配置全量工具面(与 3.40 行为一致)。offload 判定改 state.files 的解耦与 review 回改(files 非空决定能存/工具在场决定能读)保留。若未来弱模型工具选择稳定性问题实际发生,方向是**自动**瘦身(如检测到 html 子 agent 自动收敛),不再暴露开关。
>
> (原立项状态:**规划完成待实施**。优先级 P1。目标仓库:zhuanti-agent(SDK)。)
