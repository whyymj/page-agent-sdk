...
npm run test          # 自测(tsx 跑 src/__tests__/selftest.ts,2882 项断言)
npm run test:e2e      # 集成层 e2e(node 跑构建产物 dist,940 项;tests/e2e/<module>.mjs 按模块拆分)
...
- **完结门禁**(默认开,无开关):todos 有未完成项却欲纯文本收尾 → 回灌「双出口」反馈(已完成→update_todo 标记 / 未完成→继续)续跑,≤2 次防死循环;豁免问号收尾(向用户征询)与空 todos;挂循环条件层(transitional 后;beforeReturn 因 maxVerifyAttempts 默认 0 不跑被否决);防「拆 3 项做 1 项就收口」的莫名中断。**收口门禁族已抽 gateChain.ts**(transitional→完结→evidence 审计→零工具→状态询问→EXHAUSTED 五层判定/文案/预算集中一处);完结门禁豁免子 agent 栈(html 子 agent planning=true 装 todos,旧「子栈无 todos」假设不成立)
...
```bash
npm test    # tsx 跑 src/core/__tests__/selftest.ts,2882 项断言
```
...
```bash
npm run test:browser  # 102 项;也可 /browser-test 斜杠命令
```
**原理**:`tests/browser/_helpers.ts` 的 `mockLlm()` 用 `page.route()` 拦截 LLM API 端点,按脚本返回 SSE 流,使 agent ReAct 循环确定性走完,不依赖真 LLM。**双协议**:同时拦截 OpenAI 兼容(`**/chat/completions`)与 Anthropic Messages API(`**/v1/messages`),各返对应格式 SSE,共享 script 计数。spec 按 demo 拆分(complex-demo 23 / page-demo 18 / html-page 8 / customize 7 / i18n 6 / icons 6 / images 6 / header-labels 5 / rag 4 / nested 3 / lifecycle 3 / queue 3 / scrollbar 3 / error-recovery 2 / human-confirm 2 / xss 2)。写新测试模板见 `.claude/skills/browser-e2e-testing/SKILL.md`。
...
#### 新增功能测试同步约定(强制)
每新增功能/配置项/导出 API,**必须同步补测试**(同 commit),至少 1 条「正常工作」+ 1 条「边界/错误」。判定:selftest = 底层纯函数/工具逻辑/中间件 hooks;e2e = 顶层返回对象方法/AgentCore/新 capabilities/新导出/inspect 反射。命名以 `✓` 开头写「功能名 → 预期行为」。**计数同步**:更新本文件断言计数(2882/940/102)与 README 中英文。自检:`npm test && npm run build && npm run test:e2e` 三绿方可提交。
...
```
- **capabilities**:默认开 `dataOps`/`fetch`/`planning`/`skills`/`vfs`/`summarization`/`memory`/`subagent`/`focus`/`workingMemory`/`missionAnchor`/`contextInspector`/`inspectEnv`;opt-in `verify`/`domInspect`(get_dom 常驻 + dom_search/dom_info 经 dom-inspect skill 按需注入)/`automation`/`agentCompression`/`skillHostScript`/`draftWrite`/`preferences`(跨会话用户偏好记忆,preferenceStore + 三层信号捕获 + pin 段注入,usage-guide §6.16)/`bulkGuard`(大批量变更门禁,须同时配 approval;量纲 = 现有组件节点数,超阈挂确认/observe 无人值守,`bulkGuard:{threshold,timeoutMs,mode}` 细配)
