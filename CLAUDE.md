...
通用「JSON 操作助手」(dataOps 场景)。systemPrompt 由 `createChatSdk({ systemPrompt })` 注入,不硬编码业务身份。
- **默认 systemPrompt(4.16 能力感知)**:用户不传时按能力选身份 —— dataOps(有 data 声明)→ `DEFAULT_SYSTEM_PROMPT`(JSON 操作助手:身份 + 能力概述 + `systemPromptHelpers.reliableWriteRules`,`---` 分隔);dataOps:false + domInspect → 「页面内容助手」(read_page/get_dom/dom_search/dom_info 能力面 + 引用块引导 + **作答车道**(answer-intent-lanes,2026-09-20 真机 dump 驱动:三车道 A 页面事实 / B 概念术语·页上没有也必须直接解释〔「本页未提及,以下是通用解释」〕/ C 求观点经验·页面没写不构成不答的理由 + 出处密度收敛〔只页面断言标出处,miss 压一句〕;`systemPromptHelpers.answerLanes(+En)` 同源导出,自定义身份宿主〔文档问答类〕拼装复用 —— 修单车道 grounding 把概念解释压成 RAG miss 报告、求观点压成页内引文索引;**页面断言门禁不动**,只放开不引用页面的作答);take_screenshot 行仅实装配时出现);双无 → 通用兜底(与 createAgent 兜底同文案)。用户传了完全覆盖。`appendReliableWriteRules` 默认 `true`:自定义 systemPrompt 末尾自动追加写入规则(**dataOps:false 时不追加** —— 勿教池里不存在的写入工具,确需自行拼 reliableWriteRules);设 `false` 关闭(用默认 prompt 时无效,默认已含)。`createAgent` 层兜底 `'你是一个智能助手。'` **输出纪律(4.19,真机驱动)**:页面分支默认身份 + `page-analysis` skill「输出纪律」段明确禁四类无用内容 —— 过程叙述(「我先读一下页面」)/ 元话术(「先给结论」「依据是」)/ 方法论自述(解释结论怎么推出来的)/ 预告套话(「下面分三点」「综上」),出处用最简形式(句末括注「(§小节名)」);此前 SDK 只管「不猜测」不管「不啰嗦」
- **职责分工(重要)**:内置工具用法(read/write/get/set/patch/snapshot/query/search/eval 等)由 `usageHints` 中间件按能力开关自动注入运行时 prompt,**集成方 systemPrompt 只写业务知识**(身份、字段含义、业务流程、技能引用),不要重复声明工具语法。**动态组件说明/按运行时状态注入**走 `augmentSystem({ state, data })` 钩子(每轮调,setData 后 data 自动同步),见 `doc/system-prompt.md` B6 段。**⚠️ 幂等契约(4.18,S1)**:augmentSystem / 自定义 augmentPrompt 同一轮内会被调多次(toLC/replaceSystem/收口综合/inspect 内省),只有随请求那次生效 —— 回调必须幂等、禁推进状态/消费一次性标志(会被丢弃调用吞掉,内容永不送达),跨轮状态在整轮结束推进(`sdk.hook` done/message_update);SDK 自身 token 预算提示曾踩同族坑(4.18 修,`tokenBudgetHintText` 纯函数持续注入)
...
npm run test          # 自测(tsx 跑 src/__tests__/selftest.ts,3782 项断言)
npm run test:e2e      # 集成层 e2e(node 跑构建产物 dist,1281 项;tests/e2e/<module>.mjs 按模块拆分)
...
- **乐观锁契约**(3.32 opt-in 翻转):`read` 附 `hash=xxx`(乐观锁标识);**自动检测默认不开** —— `conflictWatchFields`(白名单,任意深度字段名,位置不敏感)声明后仅监听字段值变动触发冲突;`['*']` 通配 = 旧版全字段检测(editor_fangzhou 实测宿主每秒回写 minHeight 类元数据噪声驱动翻转;autoLock 已废弃);hash 不匹配 → 挂起 `sdk.pendingConflict` → `resolveConflict('keep_external'|'overwrite'|'restore')`;**per-scope 基线**(子不污染主);**同 scope 连续写永不互相冲突**;**并发写互锁(4.1+,C 形态)**:`maxParallelTools>1 && conflictWatchFields 武装` 相与时 dataOps 闭包级 async mutex(单锁 bind 域,主×子共享闭包)串行化全部写工具 `[取 effHash → handleConflict → commit → setBaseline]` 段(9 个 commit 位:write set/edit+patches/del/draft_commit/eval transform 三模式(**4.9.1 补齐**)+ restore_data/resource_update(**4.11.2 补齐** B10,修前旁路互锁;restore 空栈/查找一并入锁防并行抢跑误报 NO_SNAPSHOT;controller.updateResource 同步签名无法持锁留注记);effHash 在 acquire 后取保 N1 时序)—— 后写锁内取前写刷新后基线,**陈旧基线同轮双写从「后者静默覆盖」变「前者落地 + 后者被裁决恢复点校验显式拦下」**;ask 挂起拆段放锁(防饥饿,兄弟写照常)+ 裁决恢复点单发校验(锚 = 裁决者所见 hash,非 effHash —— 对 effHash 校验会把每次 overwrite 裁决都打回自我否决;**restore 裁决同款校验 4.11.2 补齐 B11**,另 ConflictInfo.snapshotId 真实锚定 + 锚 miss 显式 SNAPSHOT_STALE + 裁决分支 setBaseline 补 per-call scope);overwrite 裁决吸收基线 + restore 裁决补基线刷新;**并行双冲突自动收口留痕 4.11.2**(prev 被 keep_external 收口 → observable `CONFLICT_PREV_AUTO_RESOLVED` + debugLogs);串行/未武装直通 no-op(conflictWatchFields 仍是唯一旋钮,未武装并行「后写覆盖」为既有明文语义);**abort→keep_external 联动全入口**(stream/send/batch,flow-robustness P0#2;conflictManager.set 另接受可选 signal race,按 id 比对防 ref 深代理);**conflictPolicy**(3.29,默认 `'ask'` 挂起等人工 / `'overwrite'` agent 强制覆盖不挂起 / `'keep_external'` 自动保留外部):宿主与 agent 争同一份数据、无人值守场景声明 overwrite 防流程永挂;自动裁决仍外发 conflict 事件(`autoResolved` 标记)
- **高层 read/write**:`read` 多路径/裁剪/分页;`write` 四意图(set/patch/**patches 原子任一失败回滚**/del;patch op 枚举 set/remove/merge/append(数组 push **或字符串尾接** —— 大 code 分块写入:先 set 首块再 append 逐块拼接,单次输出脱离 max_tokens 上限,4.1+;**同批 set+append/多 append 同路径 4.9 修**:校验取 clone 批内累积终值 + set 终值已含 append 效果不再重放 —— 修前 live 非字符串时次块被静默丢弃/裸 chunk 撞 min 误拒/双写 AAABBB→AAABBBBBB 三形态)/**move**(value=目标路径字符串,数组元素同数组重排/跨数组移动一步原子))+ `dryRun`;`query_data` 批量 `queries`(4.6 W1:2-10 条一次取回,与 expr 互斥同传按 queries,逐条与单次输出同构、单条失败不整批);快照 per-path 栈 + `restore_data`(**选择性回退 4.9.1**:受保护字段 freeze/verbatim 与快照差异时保留宿主现值不回退 —— 快照存 bind 真值,整体回退会洗回旧值借 restore 绕 freeze;元素对应优先 __pgId 锚定跨调序正确,元素已删/无值保留快照值 + 警示宁旧勿错;**整体校验降级留痕 4.9.2 A3**:safeParse 失败改 audit 留痕放行 —— 快照是历史活态兄弟脏数据株连合法回退,schema 变更场景经 controller.set 清栈证死不可达,SNAPSHOT_SCHEMA_INVALID 移除)+ `history_data`(**deepClone 环防御 4.1+**:环数据抛可诊断错误含环路径 `$.a.b`,非「Converting circular structure」零线索;checkpoint save 克隆失败跳过本轮快照 + warn,不再 reject beforeModel 炸整轮);`eval_script` Worker 沙箱;`draft_*` opt-in(大 JSON 建议 maxToolRounds 20-30)。**写路径成本收敛(3.25.1)**:同调用 hash 单算(`commitBaseline`,基线+消息复用)+ codeAsset 改前态单拷贝(beforeBind 复用为快照条目,restore 防御性深拷贝兜底),1MB 单写 median -12~-22%;**不变量:冲突检查 hash 恒实时计算,禁跨调用缓存**(人工直改 reactive bind 不经 SDK 写路径,脏标记失明 → keep_external 失效)
- **工具面恒全暴露**(3.31 移除 `toolMode`/`interceptors`;4.0 legacy-crud-dedup 移除 get/set/edit/delete_data 四件;4.9 移除 `describe_data` 与 read 不传 jsonPath 等价、真 LLM 基线连续三版 0 调用):`createDataOps` 直出 9 工具,无呈现模式筛选;usageHints 无档位,提示词只随能力开关变化;受保护资源 freeze/verbatim(占位符只在读写边界替换,bind 恒持原值;**freeze 锚定「值」不锚定「数组位置」(freeze-move 豁免 2026-08-28)** —— 调序(move op/整数组 set)时保护字段原值随元素整体位移放行,注册表同步重锚定(commitReanchors 持久迁移,防调序后路径过期误报);值被改/元素被删照拦,仅同数组位移豁免(跨数组不豁免);**含保护字段的容器整体清空/替换显式拒** —— 回填分支按「容器仍在→回填 / merge 根键未传→skip 不捏造 / 根键在场链断→FROZEN_FIELD 带出路」三路,修「set components=[] 捏造骨架→SCHEMA_INVALID 不提保护→模型 15 轮摸路」+ merge 模式骨架覆盖原数组的静默丢失;resource_delete/list 对静态 freeze 给定向文案不误导);**vfs 四池** LRU + 大结果外存(**当轮创建恒保护 2026-09-09**:`setProtectedRefs` 记 invoke 起点水位,`updatedAt ≥ 水位` 的 large_results 同轮 LRU 豁免 —— 修前单轮连续大整读会把本轮早前 offload 淘汰致同轮 vfs_read 404;OOM 1.5x 仍强删;未注入引用集零行为变化)
...
- **完结门禁**(默认开,无开关):todos 有未完成项却欲纯文本收尾 → 回灌「双出口」反馈(已完成→update_todo 标记 / 未完成→继续)续跑,≤2 次防死循环;豁免问号收尾(向用户征询)与空 todos;挂循环条件层(transitional 后;beforeReturn 因 maxVerifyAttempts 默认 0 不跑被否决);防「拆 3 项做 1 项就收口」的莫名中断。**收口门禁族已抽 gateChain.ts**(transitional→完结→evidence 审计→零工具→状态询问→EXHAUSTED 五层判定/文案/预算集中一处;**EXHAUSTED 口径对齐 4.11.2**:完结门禁耗尽补 `COMPLETION_GATE_EXHAUSTED` observable〔修前静默放行〕+ 零工具 EXHAUSTED 补句尾问号豁免〔回灌耗尽后模型改为征询不再误报谎报〕+ 三类 gate observable 统一进 debugLogs〔修前只 onEvent〕;transitional 句尾问号豁免 4.1+ 与完结/零工具口径对齐 + **用户问句意图豁免(2026-09-02)** —— 用户末条是问句时纯文本作答合法不回灌(nested-demo 实测:说明表格含「write/写入」被判行动叙述,回灌逼出未被要求的页面改动;detectActionImperative 尾语气词豁免同补么嘛) —— 方案征询「你选哪套?」不回灌,不与方案先行 RHC 冲突);完结门禁豁免子 agent 栈(html 子 agent planning=true 装 todos,旧「子栈无 todos」假设不成立)
- **零工具收尾门禁**(imperative-zero-tool-gate,默认开无开关):完结门禁只盯 todos,「拆 0 说做完」(不建 todos 直接谎报已完成)绕过它 —— 三要素 AND(操作祈使句〔首子句 16 字窗口动词锚定 + 只读反例/问句/免操作词/示例词(示例|例子|示范|样例,4.11.2 窗口级)豁免〕+ 本轮零等效写〔writeCapable 口径 + use_/spawn 委派计等效写,**被拒委派(返回 ERROR: 回灌如 COMPONENT_BUSY)不算等效写但事实清单如实标注「其中 N 次被拒未生效」,4.9.1**〕+ 纯文本非问句收尾)→ 回灌**事实清单**(harness 本地统计:工具计数/成功写路径/write×0/todos 完成度,机制供给事实防嘴硬)+ 三出口;出口①机械化(收口含 jsonPath/组件 id 不二次回灌);**出口③机械化(2026-09-09)**:`declaresNoAction` 诚实未做声明豁免(否定完成态「未…修改/已按要求停止/无法完成」+ 无完成态断言词 → 不回灌、EXHAUSTED 不误报;RHC 拒绝后如实收口不再烧满预算,混合声明不豁免);预算 ≤2 超限放行 + `ZERO_TOOL_GATE_EXHAUSTED` observable;子 agent 不装(`CreateAgentOptions.__pgIsSubagent`);debugLogs `stage:'zero_tool_gate'`;与 resumeNotice 双保险
...
- **主×子协同**:per-scope 基线 / allSettled 逐任务结算 / 子 usage 回传 `sdk.usage` / `subagent.timeoutMs`(默认总时长 30min,2026-08-28 抬升对齐流总时长,可覆盖/0 关;超时 abort 子流 + recoverable 回灌;`inspect().subagent.timeoutMs` 反射);观察层 `inspect().subagent.{active,history}` + DebugDrawer tab
...
- **宿主集成契约与页面问答可靠性(4.18,host-integration-contract)**:① **`sdk.notifyHostChange({reason?})`**(S2)SPA 换文/路由切换后调 —— 流内页面读五工具(read_page/dom_search/dom_info/get_dom/take_screenshot)结果置过期占位(epoch 水位把守时序,通知后新读不失效)+ 一次性「须重读」pin 段(afterAgent 清除;数据槽读不受影响;`inspect().hostReadsInvalidated` 累计);② **页面断言零依据门禁**(S3,gateChain 4.5 层)「本页写了/原文提到…」× 本轮零页面依据(含截图)× 非诚实不存在声明 → 回灌先读再断言(独立预算 ≤2 + `PAGE_ASSERTION_GATE_EXHAUSTED`;**仅 domInspect 开启装配** —— 数据槽「页面上已改成…」误伤路径结构切断,不新增配置项;`inspect().gates.page_assertion_gate` 键存在性 = 装配反射);③ **`MessageQuote.anchor`**(S4)划词捕获记录选区位置(块级 selector/块内偏移/出现序号/最近标题/捕获时 URL),引用块附 `[位置: …]` 元信息行供 `read_page({selector})` 直达;捕获与发送 URL 不一致标「锚点属于另一文档」;锚点是提示不是保证(失效回退 dom_search);④ **A8 三处门控补齐**(page-analysis withVfs / dom-inspect withDataOps / usageHints domInspect 行 dataOps×hasActions 三态,勿教不存在工具);⑤ **A9 可观测性** `inspect().gates`(各门禁 retries/exhausted)+ `inspect().systemSegments`(段名/字节/dropped 标记,修前超预算 drop 仅 console.warn 零可观察)
- **宿主导航自动报案(4.21,auto-host-watch)**:①顶层 `hostWatch`(配置即开关,`true` = 原生 hashchange/popstate;pushState patch **多层装卸安全 = patch 标记携带链根、还原回链根,防错序还原出死层**/title observer 两 opt-in;去抖 300ms 合并且 url 类优先 + `ignore` 钩子;**服务端零冒犯是一等约束** —— 逐 API 特性探测静默 no-op,`inspect().hostWatch` 反射装配态)→ 触发即转 notifyHostChange 复用 S2 全链路零新失效逻辑;watcher 为依赖注入纯工厂(`sdk/hostWatcher.ts`,SSR import 安全);②**dom_edit/dom_restore 落地 → 旧页面读占位失效**(默认开无开关,不注 pin 段 —— agent 经手的写工具结果已带改了什么;dryRun 不触发)
- **内容提案-评审-应用(4.22,content-proposals)**:顶层 `proposals`(配置即开关,未配置零注册)装配 `read_content`(基底+hashContent)/`propose_content`(`baseHash` 基底锚定漂移显式拒 + **增量 ops**:replace/insertAfter·Before/append 字面锚唯一命中、顺序应用原子整批拒;content 全量二选一);模型零写权限(onProposal 非阻塞回灌,看门狗打标);`sdk.resolveProposal(id,'applied'|'discarded',detail?)` 裁决闭环(proposal_pending/resolved 事件 + proposalNotice pin 段一次性结局注入);去重(同基底同产物拒且不动在审)+ maxPending 替换留痕;read/propose 自动并入 4.20 语义集(readsHostState/deferredWrite);`lineDiff/applyProposalOps/hashContent` 纯函数双侧导出;demo `examples/proposals-demo` + browser e2e + 真 LLM 套件 `proposals`(tests/runtime/proposals-real-llm.mjs,增量 vs 全量 token 对比)
- MCP 远程工具(**逐 server 渐进注入**:各 server 连接落定即注入工具,坏 server 的 3 次连接重试不再拖累好 server;连接重试 3 次递增退避吸收上游瞬时 502/断连 + 握手 15s 降级 + **callTool 超时闸 60s**(`callTimeoutMs` 可调,3.6+;超时回灌自纠不断连))/ Verify 自检 opt-in(`createWriteBackCheck` + adversarial)/ `get_dom` opt-in / `inspect_env` 默认开 / actions 宿主动作(**两语义标记 4.20+**:`readsHostState` → action 旧结果随 notifyHostChange 置过期占位 + 页面断言门禁依据计入;`deferredWrite` → 零工具门禁事实清单注记「待用户确认后才生效」防提案后谎报完成;未标记零行为差) / SkillSpec.exec(一次性)vs tools(反复查询)勿双轨 / Approval(挂起/收口双留痕 `approval_pending`/`approval_resolved` 2026-09-02,exportDiagnostics 可诊断「卡在等确认」;无响应自动拒,4.1+ 中间件级默认 30s:approval_request 事件带 `hold()`,响应方收到即调则不限等人;headless/send/batch/streaming:false 等无响应方路径 30s 自动拒 + `APPROVAL_AUTO_REJECTED` observable;`approval.timeoutMs` 覆盖,Infinity=关;humanConfirm 同口径)/ Checkpoint 每轮存档 / Automation 预算 + `sdk.batch` / **图片输入 image-input-vision**(三入口 📎/拖/贴 + 压缩闸 ≤1568px/≤4 张/20MB;三分支:多模态主模型(modelCaps 查表或 `llm.vision:true`)直发 content parts / 纯文本 + `images.describe` 转述注入(图不直发)/ 都不配诚实拒绝;`images.upload` 原图换 URL;持久化 thumb+vfsRef 轻形态;详见 usage-guide §6.17)/ **方案确认留痕 `lastPlanConfirmation`**(RHC 带 options 的方案被点选 → `{at,summary,choice,viaOptions}` 记录;仅方案确认记录(允许/拒绝/无 options 不写);随 SessionSnapshot 持久化跨刷新存活;switch/reset 清除;ApprovalBar 上下文提示行不自动跳过;`inspect().planConfirmation` 反射;bulk-change-guard 豁免的公共接口)/ **bulk-change-guard 大批量门禁**(`capabilities.bulkGuard` 默认关 + 须配 approval 才装配(否则 no-op 留痕);量纲 = 现有组件节点数(同组件多 patch 不拦/新增不计/深路径截组件粒度);超阈(默认 4)挂 approval(自带 30s timeoutMs,ctx.emit 通道);拒绝回灌 BULK_CHANGE_REJECTED;`mode:'observe'` 无人值守;豁免:lastPlanConfirmation + 会话级同形态一次 + dryRun;componentWriteGuard 之内装载;`inspect().bulkGuard` 反射;缓解非根治明示)

### 对话鲁棒性(详见 architecture.md §⑮)
- **三档错误模型**:recoverable 回灌自纠 / fatal emit+中断 / observable 记录;导出 `routeError`/`asAgentError`/`agentError`
- 重试:网络/429/5xx 指数退避(maxRetries=2);4xx 与 abort 不重试;⚠️ **先排除 abort 再判 status**;abort 保留 partial;**retry-visibility(2026-09-09)**:内层 LangChain 客户端 `maxRetries` 恒 0(5 构造点 = constructLlm×2/proxyLlm×2/createAgent 兜底,SDK `withRetry` 独占重试,消除内外叠乘 9 次不可见重试;**边界:仅覆盖 SDK 构造路径 —— 集成方预构造 `BaseChatModel` 实例内层缺省 6 次(LangChain AsyncCaller `caller.maxRetries`;4.11.2 起装配期/setLlm 检测 >0 → warn + observable `LLM_INSTANCE_INNER_RETRIES`),实例不可改,`llmRetries` 只计外层**,排障时实例路径先看内层);body 阶段零 emit 瞬时错补递归重试(同 `_ctxRetry` 模式);重试/终败全进 debugLogs(stage `model_retry`/`model_call_failed`)+ `inspect().llmRetries`/`llmCallFailures` 会话累计 —— 网关断流不再表现为静默黑洞(环境故障 vs SDK 回归第一判据);**tool_call id 兜底回写 AIMessage(4.1+)**:provider 漏回 id 时兜底 id 同步写回 message.tool_calls(原只回填 ToolMessage → 下轮「无 id tool_call + 有 id ToolMessage」协议 400 单轮 fatal);**send/batch 落盘错误源分流(4.1+)**:store.flush 失败不再误归因 LLM fatal 触发 automation 重跑(PERSIST_FLUSH_FAILED 留痕放行,落盘交 debounce/pagehide 兜底);**send/batch 同 stream 刷新 vfs protectedRefs(4.1+)**(原只在 stream 注册 → 跨轮 LRU 淘汰被引用 large_results → vfs_read 404)
- **挂起有界收口三契约**:① 超时默认值表(approval 30s / MCP 15s / skills fetch 30s / 流停滞 `streamStallMs` 90s 抛 `StreamStalledError` 不重试;**stream 启动闸同阈值** —— `streamer.stream()` 等响应头阶段假死(fetch 默认无超时)时 stall 看门狗不覆盖,P1-7b 补;**completion 截断检测回灌(4.1+)** —— 空输出+零工具调用且 completion 达 4000+(或 stop_reason 'length')→ 单次回灌分步写入指引(实测:整页 HTML 塞进一次 write 参数撞 max_tokens → 子 agent 静默空收口 → 主 agent 无限重委派;`completion_truncated_retry` 留痕);**流总时长 `streamMaxDurationMs` 1800s(2026-08-28 抬升:100K+ 输出生成需 20min+,600s 掐死合法长生成;空转仍由 stall 90s 主防)** —— 空转帧黑洞兜底:keepalive 空转不断重置间隔计时(实测冻结 7min+ 无报错),绝对截止抛 `StreamMaxDurationError`(继承 408 不重试,重委派/重发自愈);**集成方工具看门狗 `toolTimeoutMs` 120s(flow-robustness P0#1)** —— 只管集成方注入工具(defineTool/actions/skill 工厂/rag retriever 打 `__pgWatchdog` 标),永不 settle 时 recoverable 错误回灌不杀流;内置/MCP/委派(use_<id>/spawn_*)/conflict ask 挂起是设计内等待一律豁免;**approval/humanConfirm 无响应 30s(4.1+ 中间件级,响应方调事件 `hold()` 接管后不限时;approval.timeoutMs 覆盖/Infinity 关)**;**子 agent 单次委派总时长默认 1800s(2026-08-28 抬升对齐流总时长;subagent.timeoutMs 覆盖/0 关)**;**storage flush 逐项 5s + 初始化 ready 5s(4.1+;flushTimeoutMs 可调;超时放行留痕,落盘交后续 flush/pagehide 兜底)**);② 兜底收口必留痕;③ `activeControllers` core 级,unmount/switchSession/resetSession 先 abort 全部在途流;**abort→冲突收口联动全入口**(stream/send/batch,flow-robustness P0#2):signal abort 时挂起的乐观锁冲突自动按 keep_external 收口(send 不再永不返回;conflictManager.set 另接受可选 signal race 兜底,按 id 比对防 ref 深代理);**挂起门禁期输入禁发(gate-pending,4.9.2)**:RHC/approval/冲突条挂起时内置 ChatInput 禁发送面 + `inputGateHint` 提示行(挂起期排队消息永不消费的死局消除;hold() 接管后不限时),停止按钮逃生口不受影响,正常在途流排队语义零变化,sendMessage API 语义不动;**宽度固有尺寸containment(2026-09-02)**:`.chat-dialog { contain: inline-size }` —— 消息宽内容(nowrap 表格/长单行)的 min-content 穿透滚动容器抬宿主 flex 项 min-width:auto 地板(实测 460→602 挤没相邻栏),contain 断固有尺寸链且不含 layout/paint 不建 containing block
...
### 中间件生命周期
before 类正序、after 类逆序、wrap 类洋葱。新增能力做成**中间件或工具注入**,勿硬编码进 `createAgent`。afterAgent 必跑含早退路径(4.11.2 B5:try 边界在 runBeforeAgent 之后,compressInput 抛错/systemPrompt 超预算早退不再跳过中间件清理)
...
```bash
npm test    # tsx 跑 src/core/__tests__/selftest.ts,3782 项断言
```
