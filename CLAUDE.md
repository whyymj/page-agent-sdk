...
npm run test          # 自测(tsx 跑 src/__tests__/selftest.ts,3375 项断言)
npm run test:e2e      # 集成层 e2e(node 跑构建产物 dist,1085 项;tests/e2e/<module>.mjs 按模块拆分)
...
- **完结门禁**(默认开,无开关):todos 有未完成项却欲纯文本收尾 → 回灌「双出口」反馈(已完成→update_todo 标记 / 未完成→继续)续跑,≤2 次防死循环;豁免问号收尾(向用户征询)与空 todos;挂循环条件层(transitional 后;beforeReturn 因 maxVerifyAttempts 默认 0 不跑被否决);防「拆 3 项做 1 项就收口」的莫名中断。**收口门禁族已抽 gateChain.ts**(transitional→完结→evidence 审计→零工具→状态询问→EXHAUSTED 五层判定/文案/预算集中一处;transitional 句尾问号豁免 4.1+ 与完结/零工具口径对齐 + **用户问句意图豁免(2026-09-02)** —— 用户末条是问句时纯文本作答合法不回灌(nested-demo 实测:说明表格含「write/写入」被判行动叙述,回灌逼出未被要求的页面改动;detectActionImperative 尾语气词豁免同补么嘛) —— 方案征询「你选哪套?」不回灌,不与方案先行 RHC 冲突);完结门禁豁免子 agent 栈(html 子 agent planning=true 装 todos,旧「子栈无 todos」假设不成立)
- **零工具收尾门禁**(imperative-zero-tool-gate,默认开无开关):完结门禁只盯 todos,「拆 0 说做完」(不建 todos 直接谎报已完成)绕过它 —— 三要素 AND(操作祈使句〔首子句 16 字窗口动词锚定 + 只读反例/问句/免操作词豁免〕+ 本轮零等效写〔writeCapable 口径 + use_/spawn 委派计等效写,**被拒委派(返回 ERROR: 回灌如 COMPONENT_BUSY)不算等效写但事实清单如实标注「其中 N 次被拒未生效」,4.9.1**〕+ 纯文本非问句收尾)→ 回灌**事实清单**(harness 本地统计:工具计数/成功写路径/write×0/todos 完成度,机制供给事实防嘴硬)+ 三出口;出口①机械化(收口含 jsonPath/组件 id 不二次回灌);**出口③机械化(2026-09-09)**:`declaresNoAction` 诚实未做声明豁免(否定完成态「未…修改/已按要求停止/无法完成」+ 无完成态断言词 → 不回灌、EXHAUSTED 不误报;RHC 拒绝后如实收口不再烧满预算,混合声明不豁免);预算 ≤2 超限放行 + `ZERO_TOOL_GATE_EXHAUSTED` observable;子 agent 不装(`CreateAgentOptions.__pgIsSubagent`);debugLogs `stage:'zero_tool_gate'`;与 resumeNotice 双保险
...
#### 新增功能测试同步约定(强制)
每新增功能/配置项/导出 API,**必须同步补测试**(同 commit),至少 1 条「正常工作」+ 1 条「边界/错误」。判定:selftest = 底层纯函数/工具逻辑/中间件 hooks;e2e = 顶层返回对象方法/AgentCore/新 capabilities/新导出/inspect 反射。命名以 `✓` 开头写「功能名 → 预期行为」。**计数同步**:更新本文件断言计数(3375/1085/153)与 README 中英文。自检:`npm test && npm run build && npm run test:e2e` 三绿方可提交。
