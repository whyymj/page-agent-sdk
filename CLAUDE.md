...
npm run test:e2e      # 集成层 e2e(node 跑构建产物 dist,832 项;tests/e2e/<module>.mjs 按模块拆分)
...
- **toolMode**:`advanced`(默认 14,全暴露)/`simple`(7,主推 read/write)/`minimal`(2);usageHints 提示词档位**内部自动跟随 toolMode**(无公开开关,3.30 移除 `hintsMode`;检测存量 systemPrompt「simple 模式/未暴露」措辞自动降级提示词+warn,「勿调用」单独出现不触发);**interceptors 仅守高层 read/write**(advanced 绕过);受保护资源 freeze/verbatim(占位符只在读写边界替换,bind 恒持原值);**vfs 四池** LRU + 大结果外存
...
```bash
npm run build && npm run test:e2e    # node 跑 dist 产物,832 项
```
...
#### 新增功能测试同步约定(强制)
每新增功能/配置项/导出 API,**必须同步补测试**(同 commit),至少 1 条「正常工作」+ 1 条「边界/错误」。判定:selftest = 底层纯函数/工具逻辑/中间件 hooks;e2e = 顶层返回对象方法/AgentCore/新 capabilities/新导出/inspect 反射。命名以 `✓` 开头写「功能名 → 预期行为」。**计数同步**:更新本文件断言计数(2531/832/99)与 README 中英文。自检:`npm test && npm run build && npm run test:e2e` 三绿方可提交。
