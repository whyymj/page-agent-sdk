...
npm run test          # 自测(tsx 跑 src/__tests__/selftest.ts,3079 项断言)
npm run test:e2e      # 集成层 e2e(node 跑构建产物 dist,979 项;tests/e2e/<module>.mjs 按模块拆分)
npm run test:browser  # 浏览器 E2E(Playwright + mock LLM 双协议拦截,128 项;tests/browser/<demo>.spec.ts)
```
...
```
**原理**:`tests/browser/_helpers.ts` 的 `mockLlm()` 用 `page.route()` 拦截 LLM API 端点,按脚本返回 SSE 流,使 agent ReAct 循环确定性走完,不依赖真 LLM。**双协议**:同时拦截 OpenAI 兼容(`**/chat/completions`)与 Anthropic Messages API(`**/v1/messages`),各返对应格式 SSE,共享 script 计数。spec 按 demo 拆分(complex-demo 27(含手动编辑三件套 4)/ page-demo 22 / html-page 8 / render-check 7 / customize 7 / i18n 6 / icons 6 / images 8 / header-labels 5 / rag 4 / nested 3 / lifecycle 3 / queue 3 / scrollbar 3 / error-recovery 2 / human-confirm 2 / xss 2)。写新测试模板见 `.claude/skills/browser-e2e-testing/SKILL.md`。
...
#### 新增功能测试同步约定(强制)
每新增功能/配置项/导出 API,**必须同步补测试**(同 commit),至少 1 条「正常工作」+ 1 条「边界/错误」。判定:selftest = 底层纯函数/工具逻辑/中间件 hooks;e2e = 顶层返回对象方法/AgentCore/新 capabilities/新导出/inspect 反射。命名以 `✓` 开头写「功能名 → 预期行为」。**计数同步**:更新本文件断言计数(3079/979/128)与 README 中英文。自检:`npm test && npm run build && npm run test:e2e` 三绿方可提交。
