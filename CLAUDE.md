...
- **乐观锁(`expectedHash`)+ 冲突人工介入**:`get_data` 返回值附 `hash=xxx`;`set/edit/delete` 传 `expectedHash` 启用乐观锁——若属性在 agent get 之后被外部代码/其他 agent/用户手动改过(hash 不匹配)则触发冲突。集成方传 `createChatSdk({ ... })` 时默认开启人工介入:工具挂起,`sdk.pendingConflict`(响应式 ref)置为冲突信息,内置 ChatDialog 渲染冲突条让用户三选一 → `sdk.resolveConflict('keep_external'|'overwrite'|'restore')` 收口,工具继续。headless 集成方可 watch `pendingConflict` 自建 UI。不传 `expectedHash` → 向后兼容直接写(不校验)。`DataOpsOptions.onConflict` 可独立用于 `createDataOps`(不接 ChatDialog 时自行处理)。**per-scope 基线(fix-main-sub-isolation 2.40+)**:autoLock 基线(`baselines` Map)按 caller scope 隔离 —— 主×子共享 dataOps controller 闭包,子 agent 委派期间 dataOps 工具(带 `__dataOpsScoped` marker)经 scope proxy 切 `activeScope`,子 read/write 只动子 scope 基线,**子 read 不再污染主基线**(修原:子 read 刷新共享基线 → 父过期写静默放行覆盖外部修改);委派结束清子 scope 条目。`DataOpsController.enterScope(id)→恢复函数`(嵌套安全)/`exitScope(id)`;setData/替换 bind 清全部 scope。N1 契约:autoLock effHash 在拦截器后、冲突检查时刻解析,解析→检查→提交同步无 await → 同 scope 连续写永不互相冲突(写成功即刷基线)
...
- **schema 形状自动白名单(2.4+)**:`data.schema` 为 `ZodObject` 时,顶层声明的 key 自动作为可读写白名单 —— **读路径统一深投影(fix-data-integrity 2.41+)**:`read` 整体/get_data 整体/query/search/eval 根/diff 与子路径读一律按 schema **递归**投影隐藏未声明字段(修前根级读仅顶层浅投影,声明 key 内深层未声明字段如 `config.apiKey` 整体读泄露 —— 白名单护城河唯一破口;`projectBySchemaDeep` 单一口径);`write`/`edit`/`delete` 的 `jsonPath` **逐段校验**必须在 schema 声明内否则 `PATH_DENIED`;整体 set / set_data / eval transform 自动转 **merge 语义**(只更新声明字段,未声明字段保留防误删);**`interceptors.write` 补充的未声明字段与显式传入同样被白名单挡下不写回 bind**(2.15 安全收紧:2.15.0 曾把「补充字段写回」当修复,实为白名单绕过口子,sec-21 有断言;拦截器改声明字段值仍生效)。非 ZodObject(discriminatedUnion/record/lazy)全开放向后兼容。零新增配置,集成方用 `z.pick` 子集 schema 即生效
...
- 大结果外存:工具结果超**自适应阈值**(窗口 3.5% 推导,clamp [2000,20000];未传阈值兜底 6000)转存 vfs,只留预览 + `vfs_read`/`vfs_grep` 引用;`offloadLargeResult`(`src/core/utils/offload.ts`)返回结构化 `OffloadResult`(`{offloaded, content, path, totalChars, preview, suggestedReadPlan}`,`createAgent.ts` 调用处取 `.content`,未转存时 `offloaded=false` 直传原文)
...
- **压缩 LLM 摘要异步化(fix-data-integrity 2.41+)**:`compress` 的 LLM 摘要不再同步 await(修前阻塞首 token ≤15s,恰大 JSON 长流程场景)—— **模板先行 + 后台前缀缓存**:触发时立即用索引摘要返回(零阻塞),fire-and-forget 后台 LLM 摘要入前缀缓存(`{coveredCount, text}`;older 恒从首轮起单调扩 → coveredCount 单调可比);后续压缩命中:全覆盖直接用(`llm_summary(cached)`)/ 部分覆盖 = LLM 前缀 + 新增尾部索引增量(`llm_summary(prefix)+index_tail`;indexSummarize 绝对轮号 slice 不错位);失败不污染缓存下次重试。token 成本不增(每次触发仍一次后台 LLM)。**agentCompression 的 decide(≤6s)维持同步**(opt-in 默认关;结果直接决定本次切分参数,异步化需「用旧决策」语义,推后)
...
- 纯内存上限:vfs `maxBytes`(默认 8MB,四池分池 LRU 淘汰,见上「vfs 四池分池」);对话历史 `maxMemoryRounds`(默认 30)超限压缩为摘要 system 消息
...
- `shareContext: true` 同 `id` 多实例复用同一 `AgentCore`(同页多对话框视图)。**串行闸与在途流注册表 core 级(fix-data-integrity 2.41+)**:`runSerial`/`activeControllers` 建在 core(修前实例级私有 → 双实例并发 send/switchSession 写同一 messages 裸奔),send/batch/switchSession/stream 全经 core 级串行闸;**shareContext 下生命周期收口(unmount/switchSession/resetSession/release)中止共享 core 的全部在途流**(含其他实例发起的 —— 共享状态不允许孤儿流续写;非 shareContext 行为不变)
...
- **挂起有界收口(fix-hang-and-feedback)**:统一三契约 —— ①**超时默认值表**:无响应方路径(send/batch)approval/humanConfirm **30s 自动拒**(`approval.timeoutMs` 覆盖,Infinity 留口)+ emit `APPROVAL_AUTO_REJECTED`;MCP 握手 **15s**(`mcp[].timeoutMs`)黑洞端点降级跳过不瘫 mount;skills 远程 fetch **30s**;LLM 流停滞看门狗 `streamStallMs` 默认 **90s**(chunk 间隔含等首个;`withStallTimeout` 纯函数;`StreamStalledError` status=408 不进重试;0 关)②**可见性**:兜底收口必留痕(结构化 error / warn / debugLogs)③**abort 收口**:activeControllers 注册表(**core 级**,fix-data-integrity 上移)—— `send`/`batch` 接 `signal`(P1-4 可中断);`unmount`/`switchSession`/`resetSession` 先 abort 全部在途流再收口(approval/conflict 随 signal 自动拒,无幽灵流);stop() 清排队经 `onQueuedCleared` 记 debugLogs(D-1 拍板保持清空语义)。**UI 交互确认默认仍无限等用户**(有响应方不超时,D-2 拍板)
- **resetSession 收口统一(fix-data-integrity 2.41+)**:`sdk.resetSession()`(同步,公开 API;UI ChatHeader「清空对话」同路径)= abort 全部在途流 + **收口挂起冲突**(keep_external,修前唯一不收口的生命周期路径)+ 重置全部内存态(messages/vfs/todos/memory/mission/workingMemory/focus/checkpoint/debugLogs)+ 新 sessionId + emit session_restored。**storage 关(默认)不再早退**(修前 `!store` return → mission/focus/todos 泄漏进新对话);store 相关(createSession)才按 store 门控
...
- **三层收敛**:① 目标提示(augmentPrompt 注入「## 当前精修目标」列出所有焦点);② 视野收敛(每个焦点注入 `getSchemaAtPath(schema, path)` 子树 schema);③ 范围收紧 **strict**(wrapToolCall 对写工具拦截,`jsonPath` **不在任一焦点子树** → `PATH_DENIED` 越界回灌 LLM 自纠;**无 jsonPath 整体写 = 越界拒绝**;`eval_script` 仅 transform 参与拦截,query 只读放行;读工具不限**;vfs_write/vfs_edit 不拦**——工作区文件路径非数据 scope,fix-authorization-surface)
...
- **授权与拦截面(fix-authorization-surface 2.38+)**:① **P0 装配断层修复**:传给子 agent 中间件的 `allTools` getter 指向 **agent 合并池**(`core.agent?.allTools`,含中间件工具)—— 修原局部池致 rag/html 能力包 `allowedTools` 的 vfs 工具恒不可见;② **装配期源头 filter**(`buildChildTools`/`isReservedFrameworkTool` 纯函数):子池一律排除框架/保留工具(`spawn_*`/`use_*` 保留前缀/`load_skill`/`write_todos`/`update_todo`/`restore_last_checkpoint`/`request_human_confirmation`/`*_focus`),防 spawn 自授激活 depth 链;`extraTools`(集成方显式)不过滤;③ **spawn 自授收紧**:`tools` 参数剥离写工具(写权限只能经 `writablePaths` path guard 获得);`patches` 含无 jsonPath 项(根写)→ PATH_OUT_OF_SCOPE(P1-18);④ **子栈继承主 permissions/approval 实例**(`guardMiddleware`),子的 approval_request 直通转发回主循环 handler(ApprovalBar 收口)—— 委派不再绕过人工确认/权限规则;⑤ **vfs-bridge**(`getVfsFiles`):子 offload 大结果直落主 vfs 共享池,子 vfs_* 回读不 404(P1-15)
...
- **主×子协同隔离(fix-main-sub-isolation 2.40+)**:① **per-scope 乐观锁基线**(P1-13,见数据槽乐观锁段);② **spawn_agents allSettled**(P1-14):逐任务 try/catch 结算,单个子失败不 reject 整批 —— 聚合文本按子任务逐条 `【子任务 N】✓/✗`(失败带错误摘要),主 LLM 决策处理;孤儿问题消解(无早退,所有已启动任务跑完各自结算);③ **子 usage 回传**(P1-17a):子栈 `sub-usage` 中间件 afterModel 提取(纯函数 `normalizeUsage`,与主栈 sdk-events 共用归一)→ `SubagentOptions.onUsage` → createChatSdk 累加进 `core.usage`(`sdk.usage` 含子消耗,automation tokenBudget 不再漏算;只累计不外发 usage 事件);④ **子执行超时**(P1-17b,opt-in):`createChatSdk({ subagent: { timeoutMs } })` —— 链式 AbortController(父 signal→子)+ timer race,超时 abort 子流 + 错误回灌 recoverable(主 LLM 可重试/拆小子任务);默认关(长任务子 agent 不误杀)。新增导出 `normalizeUsage`;模块级导出 `wrapWithScope`
...
```bash
npm test            # tsx 跑 src/core/__tests__/selftest.ts(runner),1748 项断言
```
...
npm run test:e2e    # node 跑 tests/e2e-integration.mjs(runner),495 项断言
```
...
**计数同步**:补测试后同步更新本文件「测试流程」小节的断言计数(1748/495)与 README 中英文计数,以及下方测试矩阵的「改动范围」行(若引入新模块)。
...
**markdown 渲染性能(fix-data-integrity 2.41+)**:流式回复每 delta 全文重算 marked+hljs+DOMPurify 是 O(n²),巨内容冻结主线程 —— `useMarkdown` 两层防护:①**尾随节流**:大内容(>2K 字符)≤ 每 100ms 渲染一次 + 尾沿保证流结束渲染最终态(小内容直渲,打字机体验不变);②**hljs 尺寸闸**:单代码块 > `HLJS_BLOCK_MAX_CHARS`(20K)跳高亮转义直出(hljs 是单帧大头;marked/DOMPurify 线性保留,**sanitize 永不跳过**)。MessageContent 的 DOM 增强改由 html 变更驱动。导出纯函数 `renderMarkdownHtml`/`markedToHtml`/`HLJS_BLOCK_MAX_CHARS`(仅主包,含 marked/hljs 依赖)。
...
4. **构建+自测**:按「### 测试流程」末尾「发布前必跑顺序」执行(`npm run build` → `npm test` 1748 全过 → `npm run test:e2e` 495 全过 → `npm run test:exports` 导出对齐 → `npm run test:types` 类型正确 → `npm run test:size` 体积不超阈值 → `npm pack --dry-run` 核对不含 `.env`/`src`/`examples`/笔记)
