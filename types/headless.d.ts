 ...
 */
export declare function buildSystemPrompt(opts: { systemPrompt?: string; appendReliableWriteRules?: boolean; /** 主数据操作能力(caps.dataOps && data 声明;createChatSdk 自动传,直调缺省 true = 历史行为;false 时默认身份不再是「JSON 操作助手」且不追加写入规则) */ dataOps?: boolean; /** 页面探查能力(caps.domInspect;dataOps:false 时默认身份切「页面内容助手」) */ domInspect?: boolean; /** take_screenshot 已装配(未装不教,页面身份截图行收起) */ screenshot?: boolean }): string;
