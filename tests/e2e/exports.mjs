...
export async function run() {
  ...
  console.log('[e2e:exports] dataOps 工具 source=builtin / fetch_document source=builtin')
  {
    ...
    assert(descTool === undefined, 'describe_data 已移除(4.9,inspect 工具面零残留)')
    const schemaTool = sdk.inspect().tools.find((t) => t.name === 'schema_data')
    assert(schemaTool?.source === 'builtin', 'schema_data source=builtin')
