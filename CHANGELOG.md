...
- **作答车道补「已懂不查」规则**(门户真机 dump 二轮驱动:通用概念解释仍被「先读正文」推着反复整页重读,2 轮 read_page ×3 ≈ 2.6 万字符 ×3):`answerLanes(+En)` 增通用行 —— 已是通用知识或对话里已读过的内容直接用,不为同一信息再读页面;只有要断言「页面上写了什么」时才需要读页核实(与页面断言门禁互补:门禁管「断言必须有据」,本行管「非断言不必读」)。selftest sec-31 laneKeys +「已懂不查」锁定。
...
- **主 agent 思考字数计数冻结**:渲染文本被尾部滑窗截断(`REASON_TAIL_CAP` 4000 防超长思考重渲染卡死),计数原取截尾后文本长度 → 超过 4k 后恒显示「4k」不再变化(用户实测反馈);修:消息另记 `reasoningTotal` 总量(流式 delta 累积,不截尾),计数标签用总量,与子 agent `subReasonFull` 口径对齐;旧消息无该字段回退文本长度
- **子 agent 思考块展开控件**:摘要行原只有 ▸ 箭头暗示可展开,改为行尾「展开/收起」文字链右置(与主 agent 思考块口径一致;i18n 复用既有 expand/collapse 键)
- browser +2(主思考计数 4.6k 不冻结 / 子思考「展开→收起」切换)
...
- **错误建议键集收紧到 schema 声明键**:read 报错分支的父级键集建议原从原始 bind 取键,理论上会列出 schema 未声明键的**名字**(正常读路径有深投影不泄露,错误分支提前返回绕过了投影)—— 改为 schema 声明键优先,开放节点(record/any 等正常读本就放行)才回落 bind 实际键;与「未声明不泄露」护城河同口径
- **同参重复检测堵字段重排绕过**:判定 key 改为参数键排序规范化后再序列化(模型把同参字段换个顺序发出不再绕过检测;数组顺序保留 —— 顺序不同语义确不同)
- **路径下标形态中心归一(误伤修复)**:`components[0]` ≡ `components.0` —— 此前写侧用括号形态、读/证据侧用点分形态会被判「不重叠」(stale-read 失效漏配 + evidence 审计误伤同根);normalizePath 中心归一,读/写/证据三侧同口径
- **完结门禁 rider 只列本 invoke 翻转项**:todos 跨轮持久化,上一任务遗留的「已完成但 evidence 空」项不再在本轮回灌里发难(与 A2 审计面同款口径)
- **optional 字段缺值不报错**:schema 声明为可选的字段当前无值是合法状态,read 保持温和 `(undefined)` 输出(不进失败计数/不触发同参 streak);必填字段缺值或开放节点才报 `PATH_NOT_FOUND`
