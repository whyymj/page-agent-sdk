...
- **Runtime swap**: `sdk.setData(config)` / `sdk.getData()` (replaces old add/remove/listDataSlots); emits a `data_change` event after the swap (operation:'set', same contract as `importData` — non-reactive bind hosts rely on it to re-render)
...
- **HTML** (generation, 3.0 single-mode breaking): planning (`write_todos`/`update_todo`) + **code as a data asset** (`data.<writablePath>[i].code` field, persisted with the data JSON to the server DB; UI binds `data.code` directly for reactive render) + **vfs as an edit working copy** + scoped write (`writablePaths` path guard). **Framework auto checkout/commit** (main agent transparent): beforeAgent checks `data.code` out to vfs (`html/<__pgId>.html`, overwrite-refresh) → subagent edits vfs → afterAgent incrementally commits touched vfs back to `data.code` (direct bind mutation, bypassing write — no snapshot stack). `summarization` on by default (frequent code edits accumulate fast).
  ...
  - **Built-in design-taste skill `design` (html-design-skill, 4.7+, mounted by default)**: the html subagent gets a second built-in skill by default — `web-design-engineer` (vendored from [ConardLi garden-skills](https://github.com/ConardLi/garden-skills/tree/main/skills/web-design-engineer) v1.2.2, MIT) — complementing `html-fragment` (mechanics): **design owns taste** (declare the design system before coding / anti-AI-cliché rules (purple gradients, emoji icons, default Inter) / perceptually-uniform oklch palettes / 25 named style recipes (linear/apple-hig/muji/pentagram/bloomberg-terminal…) / 5-dimension critique). **Progressive disclosure**: the system index grows by one line (name+description); the 33K main doc only enters context when the subagent calls `load_skill('web-design-engineer')`; the 29 reference docs (recipes/methodology) load individually on demand via `load_skill(name, ref)` — never wholesale. The main doc is adapted in three places for the delegation context (no-network facts → trust the task + flag unverified specs in the closing report / cannot ask the user → conservative defaults + stated assumptions / output → self-contained component); references stay verbatim upstream. `design:false` disables (zero injection); `design:SkillSpec` replaces with a custom version; when you pass `skills`, design is still appended after them (no conflict). The orchestration prompt gains a recipe-name hint (visual anchors like "Linear style: warm dark + hairline detail"); upstream upgrades: `node scripts/gen-design-skill.mjs <skill-dir>` refreshes references + manually re-align the main-doc adaptations
  - **Component craft notes `craftNotes` (on by default)**: the html subagent appends a `[note] <one-line essence>` line to its final reply (htmlSystemPrompt convention); the framework's afterAgent extracts and persists it to the component's `__pgNotes` (FIFO ≤5 × 200 chars, travels with the data JSON — persistent across sessions). On the next delegation to the same component, the file map injects the latest note (`📝 notes×N`) — **design intent persists across delegations** ("handoff from the previous maintainer": design decisions / user preferences / pitfalls); state lives in the data, not in the subagent instance (same philosophy as code-as-data-asset). `__pgNotes` rides the `__pg*` sidecar mechanism (hidden from agent read projection, unwritable by the agent, framework-owned); opt out via `craftNotes:false` (zero persistence, zero injection). **Note gate (4.8.1)**: if the final reply lacks a `[note]` line, a beforeReturn feedback round asks the subagent to append one (rides the formatCheck verify chain tail, sharing the `maxVerifyAttempts` budget; the prompt-only convention has a high miss rate in practice, so the mechanism backstops it)
...
- **⚠️ `augmentPrompt` (and the `augmentSystem` option) idempotency contract (4.18)**: within a single round the hook is invoked **multiple times** (initial assembly / per-round re-render / wrap-up synthesis / `inspect()` introspection), and only the invocation whose output ships with the request takes effect — the callback must be **idempotent** (same round, same result). Never advance state or consume one-shot flags inside it: a discarded invocation swallows the flag and your content never reaches the model. Advance cross-round state (e.g. a "user switched document" warning anchor) at round end instead — listen to `done`/`message_update` via `sdk.hook` and keep the callback read-only. See `doc/system-prompt.md` §5④ for details.
...
- The **English default systemPrompt** is exported separately: `DEFAULT_SYSTEM_PROMPT_EN` + `systemPromptHelpers.reliableWriteRulesEn` (handy when writing your own English prompt); the English answer lanes ship as `systemPromptHelpers.answerLanesEn` (lanes A/B/C aligned line by line)
...
- The write-approval diff preview is documented alongside approval (`approval.preview: true`): a read-only dryRun preview attached to the `approval_request` payload, rendered by ApprovalBar as structured old→new; validation failures are visible before you approve. A custom-tool approval preview is available via `approval.previewWrite` (4.24.1): it takes precedence over the built-in write preview — use it when the write target is determined by host state rather than args (e.g. "highlight the passage the user currently selected"), returning `ApprovalWritePreview | null` whose items render on the confirm bar; `null` falls back to the built-in write preview (when `preview: true`). Before 4.24.1 this option was silently dropped by the assembly layer.
- Fully runnable examples: `examples/page-demo` has quickActions / sessionTransfer / canvas-component drag-focus enabled **by default** (draggable components + `data-path` anchors → `onDropElement` mapping); minimal-demo's `?quick=1/?transfer=1/?drop=1` are e2e switches
...
**Host-change notification (`sdk.notifyHostChange()`, 4.18)**: in an SPA, navigating/route-switching/tab-switching does **not** reload the page — everything the agent previously read from the page is now stale. Call `sdk.notifyHostChange({ reason?: string })` from your navigation handler (e.g. `reason: 'user switched to "Data Structures"'`):
- **In-flight effect**: within a running turn, page-read tool results (`read_page`/`dom_search`/`dom_info`/`get_dom`/`take_screenshot`) are replaced with expiry placeholders before the next model call (guiding a re-read; fresh reads made after the notification are untouched). Data-slot `read`/`query_data`/`search_data` results are **not** affected (page changes don't invalidate data slots).
- **Cross-turn effect**: injects a one-shot "host page has changed, re-read before answering" system segment (next invoke, pin-segment survives compression, auto-cleared at end of turn — stops the model from answering from its memory of the previous answer).
...
**Focus-anchored framing (4.22+)**: when focused, **omitting the selector defaults to screenshotting the focused component** — the SDK probes the host DOM for a `[data-path="<focus path>"]` anchor (the generic low-code convention where component DOM carries data-path; pick-to-select in complex-demo/editor-style hosts uses exactly this attribute). Hit = precise capture (result notes "framing = focused component"); miss = viewport fallback with a hint to pass a selector manually; an explicit selector/fullPage is never overridden. Referencing questions ("what is this / what does this show") get framed this way, and the answer centers on the focused component (no whole-page rambling). Override the mapping with `screenshot.focusSelector: (focusPath) => string` if your host uses a different convention.

**`view_image({ url })` — direct image-URL viewing (4.23+)**: auto-assembled whenever a visual consumer exists (vision main model or `images.describe`), **no domInspect required**. Images read from page data (a specific carousel slide / product image / cover) get their URL passed to the model directly — full-resolution original, zero rendering, fetched model-side (the whole class of CORS / canvas taint / autoplay-already-switched-frames / render failures simply doesn't exist). Split with take_screenshot: **"what does the Nth image show" with the URL in hand → view_image first**; only screenshot when you need the *rendered* component (layout / overlay text). Non-vision models automatically go through describe captioning; http(s) URLs only.
...
- Fully runnable example: `examples/eval-demo` (regression panel: fixed scenario prompt → run once → report + business assertion → save baseline to localStorage → diff ▲▼ next time; copy it for your host)
