...
| Doc | Contents |
|---|---|
| [**Usage Guide**](./usage-guide.en.md) | **Start here** · Install / quick start / options / capability deep-dive / **UI customization & i18n (6.15: icons / theme + top-level `i18n` group — locale switch / per-key message overrides)** / **cross-session preference memory (6.16)** / **image input (6.17: multimodal direct / captioning bypass via `images.describe` for text-only models)** / **diagnostics export (6.13c: `exportDiagnostics` one-click full log report)** / custom middleware / FAQ |
