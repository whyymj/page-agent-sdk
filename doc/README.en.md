...
| Doc | Contents |
|---|---|
| [**Usage Guide**](./usage-guide.en.md) | **Start here** · Install / quick start / options / capability deep-dive / **UI customization & i18n (6.15: icons / theme + top-level `i18n` group — locale switch / per-key message overrides)** / **image input (6.17: multimodal direct / captioning bypass via `images.describe` for text-only models)** / **quick actions & session transfer (6.18: `dialog.quickActions` one-click prompts / session export-import / element drop-focus + `approval.preview` diff)** / **selection quoting & page QA (6.20: `dialog.autoQuote` quote-chip questioning + `read_page` paginated body reading + `pageContext` page anchor)** / **screenshot viewing & page analysis (6.21: `take_screenshot` three modes + layered image channel + `page-analysis` skill; `?shot=1` demo)** / **diagnostics export (6.13c: `exportDiagnostics` one-click full log report)** / custom middleware / FAQ |
...
- **Self-tests**: `npm test` (`../src/core/__tests__/selftest.ts`, 3473 assertions) + `npm run test:e2e` (integration e2e, 1106 assertions) + `npm run test:browser` (browser E2E, 153 assertions)
