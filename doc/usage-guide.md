...
- **运行时替换主数据**:`sdk.setData(config)` / `sdk.getData()`(替代旧 add/remove/listDataSlots);替换后发 `data_change` 事件(operation:'set',与 `importData` 口径一致 —— 非 reactive bind 宿主靠该事件驱动重渲染)
...
- **HTML**(生成型,3.0 单模式 breaking):规划(`write_todos`/`update_todo`)+ **代码作为 data 资产**(`data.<writablePath>[i].code` 字段,随 data json 持久化进服务端 DB;UI 直接绑 `data.code` 响应式渲染)+ **vfs 作编辑工作副本** + 限定写(`writablePaths` path guard)。**框架自动 checkout/commit**(主 agent 透明):beforeAgent 把 `data.code` 按 `__pgId` 检出到 vfs(`html/<__pgId>.html`,覆盖式刷新)→ 子 agent 在 vfs 改 → afterAgent 增量回写改过的 vfs → `data.code`(直改 bind,不经 write,不进快照栈)。默认开 `summarization`(频繁改代码累积快)
  ...
  - **内置设计品味 skill `design`(html-design-skill,4.7+,默认挂载)**:子 agent 默认并列挂第二个内置 skill `web-design-engineer`(vendored 自 [ConardLi garden-skills](https://github.com/ConardLi/garden-skills/tree/main/skills/web-design-engineer) v1.2.2,MIT)—— 与 `html-fragment`(落地规范)分工:**design 管品味**(设计系统先声明再写码 / 反 AI 俗套规则(紫渐变/emoji 图标/Inter 默认)/ oklch 感知均匀配色 / 25 个具名风格配方(linear/apple-hig/muji/pentagram/bloomberg-terminal…)/ 5 维自评)。**渐进披露**:system 索引只多一行(name+description);33K 主文只在子 agent 主动 `load_skill('web-design-engineer')` 时进上下文;29 个参考文档(配方/方法论)再按需 `load_skill(name, ref)` 单个取回,不整包灌。主文三处适配委派场景(无网事实→task 为准+收口报告标注 / 不能反问→保守默认+假设说明 / 输出→自包含组件),参考文件保持上游原文。`design:false` 关闭(零注入);`design:SkillSpec` 替换为集成方自定义版本;用户传 `skills` 时 design 仍追加在用户 skills 之后(不冲突)。编排段同步注入配方名引导(视觉锚可写「Linear 风格:暖深色+发丝线」);上游升级:`node scripts/gen-design-skill.mjs <新skill目录>` 刷 references + 手工对齐主文适配
  - **组件工匠笔记 `craftNotes`(默认开)**:子 agent 收口回复末尾附 `[note] <一句话实现要点>` 行(htmlSystemPrompt 约定),框架 afterAgent 提取沉淀为组件 `__pgNotes`(FIFO ≤5 条 × 200 字,随 data json 进服务端 DB 跨会话持久);下次委派同组件时经「组件代码文件地图」注入最近 1 条(`📝 笔记×N`)—— 同组件跨委派**设计意图持续**("前任的交接":设计决策/用户偏好/踩坑),状态在数据里不在子 agent 实例里(与 code-as-data-asset 哲学同构)。`__pgNotes` 走 `__pg*` sidecar 机制(agent read 投影隐藏、写不进,框架独占);`craftNotes:false` 关闭(零沉淀零注入)。**末行守卫(note-gate,4.8.1)**:收口回复缺 `[note]` 行时 beforeReturn 回灌补写一次(挂 formatCheck verify 链尾,预算与 `maxVerifyAttempts` 共享;纯提示词约定实测漏写率高,机制兜底)
