...
- **完结门禁(防莫名中断)**:agent 用 `write_todos` 规划后,若还有未完成项却试图以纯文本收尾,框架回灌「双出口」反馈让它继续(已完成→先 `update_todo` 标记;未完成→继续执行),最多 2 次防死循环。豁免:收尾是问号(在征询用户)或根本没规划过(空 todos)。修「拆 3 项做完 1 项就停」的提前中断
