...
export function htmlOrchestratorPrompt(id: string): string {
  ...
  return [
    ...
    `【主 agent 职责边界(硬规则)】禁止直接 read/write 代码组件的 code 字段(read 只得 <code Nkb> 摘要,看不懂细节没用;write 绕过 vfs/verify 无格式校验,危险)。所有代码生成/修改/排查必经 ${use} 子 agent:① 修改/排查类先 read 定位目标组件元信息(name/type/props 非 code;新建类跳过本步直接委派)② 委派 ${use}(task 写清改什么 + 转述用户反馈/现象)③ 收尾核对。不自己改代码、不自己排查代码细节。`,
