...
export function deepClone<T>(v: T): T {
  ...
  try {
    ...
    // flow-robustness P1#8:环防御诊断 —— JSON.stringify 遇环抛 "Converting circular structure to JSON" 零路径线索,
    // 写路径第一条语句即抛会使功能瘫痪且无从排查;失败时单发探测环路径,抛可诊断错误(非环失败原样上抛)
    const cyc = detectCyclePath(v)
    if (cyc) throw new Error(`数据存在循环引用(${cyc});数据槽/快照不支持环形结构,请检查宿主 bind 或写入值`)
    throw e
  }
}

/** 环路径探测(DFS,带回溯:同引用出现在不相交分支是合法 DAG,仅祖先链重复算环;返回首个环路径,无环 undefined) */
function detectCyclePath(root: unknown): string | undefined {
  ...
  const walk = (node: unknown, path: string): string | undefined => {
    ...
    if (Array.isArray(node)) {
      ...
      for (const k of Object.keys(node)) {
        const r = walk((node as Record<string, unknown>)[k], `${path}.${k}`)
        if (r) return r
      }
    }
    seen.delete(obj) // 回溯出栈
    return undefined
  }
  return walk(root, '$')
}
...
export function safeStringify(value: unknown, maxLen = Infinity, ignoreKeys?: ReadonlySet<string>): string {
  ...
  try {
    result = JSON.stringify(
      value,
      (_key, val) => {
        // 字段级豁免(conflictIgnoreFields):跳键不参与序列化 —— 任意深度同名键全跳过
        if (ignoreKeys && _key && ignoreKeys.has(_key)) return undefined
...
/**
 * @param ignoreKeys 可选字段名集合(任意深度同名键跳过序列化)——内部兼容保留;
 * 3.32 冲突检测改白名单 opt-in 后,监听模式用 watchFieldsHash。
 */
export function hashValue(value: unknown, ignoreKeys?: ReadonlySet<string>): string {
  return cyrb53(safeStringify(value, Infinity, ignoreKeys)).toString(36)
}

/**
 * 白名单监听 hash(conflictWatchFields):深度收集所有名为 watchKeys 的键值(任意深度),
 * 排序后哈希。位置不敏感 —— 组件增删/调序致 jsonPath 位移不误报;任一监听字段值变动必报。
 * 监听字段的子树整体参与(值变即 hash 变);非监听字段与结构变化不可见(默认不检测语义)。
 */
export function watchFieldsHash(value: unknown, watchKeys: ReadonlySet<string>): string {
  const parts: string[] = []
  const walk = (v: unknown): void => {
    if (Array.isArray(v)) { v.forEach(walk); return }
    if (v && typeof v === 'object') {
      for (const [k, child] of Object.entries(v as Record<string, unknown>)) {
        if (watchKeys.has(k)) parts.push(`${k}=${safeStringify(child)}`)
        walk(child)
      }
    }
  }
...
export function applyPatchToClone(clone: any, op: EditOp, jsonPath: string, value: unknown): string | null {
  if (op === 'set') {
    if (!jsonPath) return 'set 操作需要 jsonPath(整体替换请用 write({value}))'
