...
export async function run(ctx: TestCtx): Promise<void> {
  ...
  // ===== C. detectQuestionIntent =====
  {
    ...
    // 么/嘛 尾语气词 + 能/可以/会/行 中信号(2026-09-02,nested-demo 实测「你能修改嵌套层级么」漏判驱动)
    assert(detectQuestionIntent('你能修改嵌套层级么'), '✓ 问句判定 → 「能」+ 句尾「么」(nested-demo 实测事故句)')
    assert(detectQuestionIntent('这个方案行吗'), '✓ 问句判定 → 「行」+ 句尾「吗」')
    assert(detectQuestionIntent('明天会下雨么'), '✓ 问句判定 → 「会」+ 句尾「么」')
    assert(detectQuestionIntent('这里可以放视频嘛'), '✓ 问句判定 → 「可以」+ 句尾「嘛」')
    assert(!detectQuestionIntent('可以开始了'), '✓ 问句判定反例 → 「可以」无句尾语气词不命中')
    assert(!detectQuestionIntent('设计一个活动页'), '✓ 问句判定反例 → 祈使基线仍不命中')
    assert(!detectQuestionIntent('把标题改成X'), '✓ 问句判定反例 → 祈使基线仍不命中(2)')
    // 全角问号跟随「吗/呢」(team-audit P2 修:修前 QUESTION_TAIL_RE 只配半角 ?,「…吗？」漏判)
    assert(detectQuestionIntent('这是那个倒计时组件吗？'), '✓ 问句判定 → 句尾「吗」+ 全角问号(修前漏判,中信号档)')
    assert(detectQuestionIntent('为什么这里报错了呢？'), '✓ 问句判定 → 句尾「呢」+ 全角问号(同修)')
    // 反例(祈使/操作指令,必须不命中)
