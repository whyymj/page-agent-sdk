/**
 * sec-61:placeholder-protected-read-write Phase 4(跨压缩 pin)
 * resourcesPin 中间件 augmentPrompt 注入「受保护资源」段(usageHints 资源教程段已去重移除)。
 */
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function hintPrompt(): string {
  const mw = createUsageHintsMiddleware({}, true)
  return (mw.augmentPrompt as any)?.({} as any) ?? ''
}
