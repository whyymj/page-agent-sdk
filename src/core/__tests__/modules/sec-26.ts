...
export async function run(ctx: TestCtx): Promise<void> {
  ...
  console.log('\n[乐观锁 conflictWatchFields 单旋钮]')
  {
    ...
    let r = await invoke(t['read'], { jsonPath: 'title' })
    assert(/hash=/.test(r), 'read 返回含 hash(乐观锁标识)')

    // 基线新鲜 → write 直接成功(无需任何手动 hash 参数)
    r = await invoke(t['write'], { patch: { op: 'set', jsonPath: 'title', value: '"new"' } })
    assert(/已 write\(edit\)/.test(r) && pageObj.title === 'new', "['*'] 基线新鲜 → write 直接成功")

    // 外部代码改了 count(基线过期)→ write 必 CONFLICT,值不被覆盖
    pageObj.count = 99
    ...
    // 重新 read 刷新基线 → write 成功
    await invoke(t['read'], { jsonPath: 'count' })
    r = await invoke(t['write'], { patch: { op: 'set', jsonPath: 'count', value: '5' } })
    assert(/已 write\(edit\)/.test(r) && pageObj.count === 5, "['*'] 刷新基线后 write 成功")

    // 嵌套子路径(config.bg)同样受整体基线保护
    pageObj.config.bg = 'external'
    r = await invoke(t['write'], { patch: { op: 'set', jsonPath: 'config.bg', value: '"edited"' } })
    assert(/VERSION_CONFLICT/.test(r) && pageObj.config.bg === 'external', "['*'] 嵌套子路径:基线过期 → CONFLICT,外部改不被覆盖")
    await invoke(t['read'], { jsonPath: 'config' })
    r = await invoke(t['write'], { patch: { op: 'set', jsonPath: 'config.bg', value: '"edited"' } })
    assert(/已 write\(edit\)/.test(r) && pageObj.config.bg === 'edited', "['*'] 嵌套子路径:刷新基线后 write 成功")

    // delete 同样受保护:基线过期拒删,刷新后可删
    pageObj.count = 77
    r = await invoke(t['write'], { patch: { jsonPath: 'config' }, del: true })
    assert(/VERSION_CONFLICT/.test(r), "['*'] delete 基线过期 → CONFLICT 拒绝")
    await invoke(t['read'], { jsonPath: 'title' })
    r = await invoke(t['write'], { patch: { jsonPath: 'title' }, del: true })
  ...
  // 未声明 conflictWatchFields → 默认不检测(外部改后 write 直接写,不校验)
  {
    ...
    const t = byName(tools)
    await invoke(t['read'], { jsonPath: 'x' })
    appObj.x = 99                                            // 外部改
    const r = await invoke(t['write'], { patch: { op: 'set', jsonPath: 'x', value: '5' } })
    assert(/已 write\(edit\)/.test(r) && appObj.x === 5, '未声明 → 默认不检测,外部改后 write 直接写入')
  }
  ...
  // onConflict 人工介入:冲突时挂起等用户决定(保留外部/强制覆盖/回退)
  {
    ...
    await invoke(t2['read'], { jsonPath: 'x' })  // 建基线
    const tick = () => new Promise<void>((r) => setTimeout(r, 5))

    // keep_external:保留外部改后的值,不写入
    pageObj.x = 'external'
    const p1 = invoke(t2['write'], { value: '{ "x": "agent" }' })
    ...
    pageObj.x = 'external2'
    const p2 = invoke(t2['write'], { value: '{ "x": "agent2" }' })
    await tick()
    resolveC({ action: 'overwrite' })
    r = await p2
    assert(/已 write\(set\)/.test(r) && pageObj.x === 'agent2', 'onConflict overwrite → 强制覆盖,写入 agent 值')

    // restore:回退到快照栈顶(历史检查点),不写入 agent 值
    // 此前 overwrite 已 push 一条快照(external2,即 overwrite 写前值);restore 回退到它
    pageObj.x = 'external3'
    const p3 = invoke(t2['write'], { value: '{ "x": "agent3" }' })
    await tick()
    ...
    await invoke(t3['read'], { jsonPath: 'y' })
    pageY.y = 'yext'
    const p4 = invoke(t3['write'], { value: '{ "y": "ya" }' })
    await tick()
    resolveC({ action: 'restore' })
    r = await p4
  ...
  // JSON 直传(L1):value 支持直接传 object,无需 stringify;也兼容旧 string
  {
    ...
    let r = await invoke(t['write'], { patch: { op: 'set', jsonPath: 'obj', value: { name: 'b', age: 2 } } })
    assert(/已 write\(edit\)/.test(r) && appObj.obj.name === 'b' && appObj.obj.age === 2, 'write(edit) set 子路径直传 object 写入成功')
    // edit 仍兼容 JSON 字符串
    r = await invoke(t['write'], { patch: { op: 'set', jsonPath: 'obj', value: '{"name":"c","age":3}' } })
    assert(/已 write\(edit\)/.test(r) && appObj.obj.name === 'c' && appObj.obj.age === 3, 'write(edit) 兼容 JSON 字符串写入')
    // edit set 子路径直传非法 object → schema 校验失败(不写入)
    r = await invoke(t['write'], { patch: { op: 'set', jsonPath: 'obj', value: { name: 'd', age: 'not-number' } } })
    assert(/error|校验失败|invalid|SCHEMA_INVALID/i.test(r) && appObj.obj.name === 'c', 'write(edit) set 子路径直传非法 object → 校验失败不写入')
    // edit merge 直传 object
    r = await invoke(t['write'], { patch: { op: 'merge', jsonPath: 'obj', value: { age: 5 } } })
    assert(/已 write\(edit\)/.test(r) && appObj.obj.age === 5, 'write(edit) merge 直传 object 成功')
    // edit append 直传数组
    r = await invoke(t['write'], { patch: { op: 'append', jsonPath: 'arr', value: ['y', 'z'] } })
    assert(/已 write\(edit\)/.test(r) && appObj.arr.length === 3 && appObj.arr[2] === 'z', 'write(edit) append 直传数组成功')
    // edit append 仍兼容 JSON 字符串
    r = await invoke(t['write'], { patch: { op: 'append', jsonPath: 'arr', value: '["w"]' } })
    assert(/已 write\(edit\)/.test(r) && appObj.arr[3] === 'w', 'write(edit) 兼容 JSON 字符串 append')

    // write(set) 整体直传 object
    r = await invoke(t['write'], { value: { obj: { name: 'X', age: 9 }, arr: ['m'] } })
    assert(/已 write\(set\)/.test(r) && appObj.obj.name === 'X' && appObj.arr.length === 1, 'write(set) 整体直传 object 写入成功')
  }
  ...
  // write-path-cost-reduction A 段:commitBaseline 单算复用 —— 消息「新 hash」与真实当前态一致(防基线/消息漂移)
  {
    ...
    // write(edit) 同口径
    const r3 = await invoke(t['write'], { patch: { op: 'set', jsonPath: 'count', value: '4' } })
    const m2 = /新 hash=(\w+)/.exec(r3)
    assert(!!m2, '✓ write(edit) 返回含「新 hash」')
