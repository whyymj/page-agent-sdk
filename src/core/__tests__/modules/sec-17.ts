...
export async function run(ctx: TestCtx): Promise<void> {
  ...
  // bind 脏标记增量(dataOps controller:写路径标脏,dryRun/只读不标;commitSetToBind onWrite 收敛 set 类)
  {
    ...
    await invoke(names['write'], { value: { title: 'set改', count: 2 } })
    assert(controller.consumeDataDirty() === true, 'write(set) 写 bind → 标脏(commitSetToBind onWrite)')
    await invoke(names['write'], { patch: { op: 'set', jsonPath: 'title', value: 'edit改' } })
    assert(controller.consumeDataDirty() === true, 'write(edit) 写 bind → 标脏')
    await invoke(names['read'], { jsonPath: 'title' })
    assert(controller.consumeDataDirty() === false, 'read 只读 → 不标脏')
    ...
    await invoke(names['write'], { patch: { jsonPath: 'count' }, del: true })
    assert(controller.consumeDataDirty() === true, 'write(del) 写 bind → 标脏')
