...
/** 疑问词(中信号):需配合句尾「吗/呢」才判定问句(单独出现可能是祈使,如「看看为什么报错然后修一下」) */
const QUESTION_WORD_RE = /(为什么|为啥|多少|几个|能不能|可不可以|是否|有没有|怎样|咋)/
/** 查询词(高信号):此类词几乎只出现在信息咨询,命中即判问句(「这是啥组件」实测案例即此档) */
const QUERY_WORD_RE = /(是什么|是啥|怎么用|如何用|有哪些|什么意思|干嘛的|干什么用)/
/** 问句收尾:句尾「吗/呢」(允许跟问号;半角+全角 —— 全角？漏配会复制成地雷,team-audit P2) */
const QUESTION_TAIL_RE = /(吗|呢)[?？]?\s*$/
