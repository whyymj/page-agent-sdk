...
export interface DialogConfig {
  ...
  autoQuote?: boolean
  /** 划词浮动菜单(page-quote 显式确认形态,默认 false):划选宿主文字浮出「❝ 引用到 AI 助手」工具条(选区上方,fixed),点击 = 挂引用 chip + 打开对话框 + 聚焦输入;点别处/滚动/Esc 消失。与 autoQuote 独立可组合(本开关是显式确认,autoQuote 是静默捕获)。自定义三层(文案 i18n.messages / 样式覆盖须非 scoped 样式表 / 关掉本开关后 captureSelectionQuote+sdk.setQuote 自建)见 usage-guide §6.20;菜单项当前固定一项 */
