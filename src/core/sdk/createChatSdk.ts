...
import { createVerifyMiddleware, createWriteBackCheck } from '../harness/verify'
import { createContextInspectorMiddleware } from '../harness/contextInspector'
import { connectAllMcpServers } from '../mcp/connectAll'
...
import { isChatModel, resolveLlm, flagInstanceInnerRetries } from './llmResolver'
...
import { resolveContextOptions, PRESET_PRESERVE } from './contextPreset'
...
function buildCore(options: ChatSdkOptions, agentId: string): AgentCore {
  ...
  const vfsStore = createVfs(options.vfs?.initialFiles, {
    persist: store
      ? { save: (files: Record<string, VfsFile>): void => {
          sl.persistSave({ vfs: files })   // fire-and-forget 统一出口(吞错防 unhandled rejection;函数声明提升,定义在后可调用)
        } }
      : undefined,
  ...
  // F2a:会话级可变状态收敛单对象(散 let 跨作用域赋值是 P0-4 根因形态;详见 sdk/sessionLifecycle.ts)
  const sessionVars = createSessionVars()
  ...
  const humanConfirmMw = createHumanConfirmMiddleware((record) => {
    sessionVars.lastPlanConfirmation = record
    sl.persistSave({ planConfirmation: record })
  ...
  // F3:工具装配岛委托 sdk/toolAssembly(集合所有权留本文件,装配策略外移)
  const rebuildExtraTools = createToolRebuilder({
    ...
    mcpTools: () => mcpTools,
    loadedSkillTools: () => loadedSkillTools,
  })
  ...
  const subagentsForAssemble: SubagentConfig[] | undefined = effectiveSubagents?.map((s) => {
    ...
    for (const m of newMiddleware) {
      if (m && hasGetController(m)) {
        m._setGetController(() => dataOpsController)
      }
    }
  ...
  const subagentsController = subagentsMw ? subagentsMw.controller : null
  ...
  const syncUserSkills = () => {
    const ctrl = skillsMw ? skillsMw.controller : null
    if (ctrl) {
  ...
  const summarizationMw = useSummarization
    ...
    : undefined
  // F0a(struct#3 HIGH,2026-09-10 拆分前置):skillsMw 构造提升到 compose 之前 —— 修前赋值藏在 compose
  // 数组字面量的 spread 内(`...(useSkills ? [skillsMw = createSkillsMiddleware(...)] : [])`),
  // 1607/1621/1805/2405 四个读取点依赖「compose 先于读取执行」的隐含时序;F2/F3 搬移数组或拆两步时
  // 赋值时序极易踩断(评审明示「重构极易踩断」)。显式构造 + 数组只引用,时序依赖显性化。
  ...
  const core: AgentCore = {
    ...
    applySnapshot(snap: SessionSnapshot): void {
      ...
      if (snap.planConfirmation) sessionVars.lastPlanConfirmation = snap.planConfirmation
    ...
    async send(message: string, options?: SendOptions): Promise<string> {
      ...
      if (images.length) {
        await imagePipeline.stowImages(images) // url 型跳过;原图入 vfs + thumb(轻形态持久化引用锚;失败留痕不阻塞)
        await imagePipeline.describeIfNeeded(images, msg)
      }
    ...
    /** 切换会话(F2b 委托 sessionLifecycle;签名经 AgentCore 注解约束) */
    switchSession: (sessionId) => sl.switchSession(sessionId),
    exportSession: (sessionId) => sl.exportSession(sessionId),
    importSession: (data) => sl.importSession(data),

    resetSession: () => sl.resetSession(),
    ...
    stream: async (msgs, onEvent, signal) => {
      ...
      if (imgMsg?.images?.length) {
        await imagePipeline.stowImages(imgMsg.images)
        await imagePipeline.describeIfNeeded(imgMsg.images, imgMsg.content)
    ...
    getInfo(): AgentInfo {
      return {
        ...
        skills: (skillsMw ? skillsMw.controller.get() : (options.skills ?? [])).map((s) => ({ name: s.name, description: s.description })),
        ...
        planConfirmation: sessionVars.lastPlanConfirmation,
  ...
  async function resolveAndLoad(): Promise<void> {
    ...
    if (options.memory) sl.persistSave({ memory: memoryMw.get() || (typeof options.memory === 'string' ? options.memory : '') })
    void sl.refreshSessions()  // session-history Phase 6:init 载入会话后刷新历史列表(内部已吞错)
  }
    ...
    if (enableTrimLlm && summaryLlmInvoke) {
      ...
      void (async () => {
        try {
          ...
          sl.persistRuntime() // 落盘 LLM 增强版(异步,不阻塞主循环)
        } catch {
   ...
   */
  // F3:图片发送管线委托 tools/imageInput(stow 原图收口 + describe 旁路;modelCaps 为 let → getter 传活值)
  const imagePipeline = createImagePipeline({
    getConfig: () => options.images,
    getVision: () => modelCaps.vision === true,
    emit,
    getVfsStore: () => core.vfsStore,
    noteFailure: (stage, e) => sl.notePersistFailure(stage, e),
  })

  // F2b:会话持久化 + 生命周期族委托 sdk/sessionLifecycle(依赖显式传;修前闭包隐式捕获 20+ 绑定,P0-4 温床)
  const sl = createSessionLifecycle({
    agentId,
    options,
    emit,
    getStore: () => store,
    ...
    messages,
    getUsage: () => usage,
    mw: { todosMw, memoryMw, missionMw, workingMemoryMw, focusMw, summarizationMw, resumeNoticeMw, useMission, useWorkingMemory, useFocus, useAutomation },
    vfsStore,
    checkpointMgr,
    conflictResolve: (a) => conflictMgr.resolve(a),
    abortAllActive,
    titleLlmInvoke,
  })
  ...
  core.initDone = (async (): Promise<void> => {
    ...
    if (options.mcp?.length) {
      void connectAllMcpServers({
        servers: options.mcp!,
        emit,
        isReleased: () => mcpBackgroundReleased,
        core: { mcpClosers: core.mcpClosers, mcpServers: core.mcpServers, mcpFailed: core.mcpFailed, infoTick: core.infoTick },
        reservedNames: () => {
          // C1(MCP 保留字保护):开局快照非 mcp 来源工具名,冲突 → skip + warn
        ...
        pushTool: (t, label) => {
          ...
          mcpTools.push(t as StructuredToolInterface)
          return true
        },
        rebind: () => {
          allTools = rebuildExtraTools()
    ...
    if (subagentsMw) {
      subagentsMw.setReconfigureHook(() => {
