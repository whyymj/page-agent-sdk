...
// 示例 UI 规范(演示 createHtmlSubagent 挂规范类 skill 的模式:索引进 prompt,子 agent 生成前 load_skill 按需拉全文)
// 真实平台替换为设计规范文档(或 getContent 异步 fetch('/specs/ark-ui.md'));色值/间距是"死规格",
// 恰与子 agent「照规格实现,不穷举」的思考纪律同向 —— 规范越明确,装饰细节探索空间越小
export const arkUiSpecContent = `# 方舟平台 UI 规范(custom 代码组件必读)

## 色板(必须用这些 hex,禁止自造近似色)
- 主色:#7063E7(紫);主色渐变终点:#8B7FF0
- 成功:#00C562;警示:#F04848;强调金:#F7C948
- 底色:#222222(深)/ #353535(面板);正文:#E8E8F0;次级:#9A96B0

## 间距与圆角
- 栅格 8px 基数:8/16/24/32;卡片内边距 16px;组件间距 24px
- 圆角:卡片 12px;按钮/标签 8px;禁止 >16px 的大圆角

## 字号
- 页面主标题 28-32px;卡片标题 18px;正文 14px;辅助 12px;行高 1.5

## 组件形态约束
- 优惠券:顶部撕边用 repeating-linear-gradient 一行实现(锯齿宽 12px),禁止用 SVG 波浪或 clip-path 穷举
- 倒计时:等宽数字 font-variant-numeric: tabular-nums;背景 #353535 圆角 8px
- 徽标/戳:旋转 -6°,金色描边 1px,禁止多层阴影叠加
- 装饰密度:每组件装饰类元素 ≤2 类(如灯串+光斑),禁止堆叠第三类
- class 一律加组件名前缀(如 .cpn-coupon-),防样式冲突

## 可访问性底线
- 文本对比度 ≥4.5:1(深底上用 #E8E8F0 正文,禁 #9A96B0 做正文)
- 可点元素 min-height 44px;图片 alt 必填
`
